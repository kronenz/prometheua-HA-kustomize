...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   엣지 클러스터 (197) 배포 스크립트   ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-197-edge/longhorn --enable-helm > /tmp/longhorn-197.yaml

echo "Longhorn 배포 중..."
kubectl apply -f /tmp/longhorn-197.yaml
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-197-edge/ingress-nginx --enable-helm > /tmp/ingress-nginx-197.yaml

echo "Ingress-nginx 배포 중..."
kubectl apply -f /tmp/ingress-nginx-197.yaml
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-197-edge/prometheus --enable-helm > /tmp/prometheus-197.yaml

echo "Prometheus + Thanos Sidecar 배포 중..."
kubectl apply -f /tmp/prometheus-197.yaml
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-197-edge/opensearch --enable-helm > /tmp/opensearch-197.yaml

echo "OpenSearch 배포 중..."
kubectl apply -f /tmp/opensearch-197.yaml
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-197-edge/fluent-bit --enable-helm > /tmp/fluent-bit-197.yaml

echo "Fluent-bit 배포 중..."
kubectl apply -f /tmp/fluent-bit-197.yaml
...
echo "  - Grafana: http://grafana.mkube-197.miribit.lab:30080"
echo "  - Alertmanager: http://alertmanager.mkube-197.miribit.lab:30080"
echo "  - OpenSearch: http://opensearch.mkube-197.miribit.lab:30080"
echo "  - Longhorn UI: http://longhorn.mkube-197.miribit.lab:30080"
echo ""
echo -e "${YELLOW}Thanos Sidecar NodePort:${NC}"
echo "  - gRPC: 192.168.101.197:30901"
echo ""
