#!/bin/bash
# 엣지 클러스터 (198) 배포 스크립트
# Longhorn, Ingress-nginx, Prometheus+Thanos Sidecar, OpenSearch, Fluent-bit 배포
...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   엣지 클러스터 (198) 배포 스크립트   ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-198-edge/longhorn --enable-helm > /tmp/longhorn-198.yaml

echo "Longhorn 배포 중..."
kubectl apply -f /tmp/longhorn-198.yaml
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-198-edge/ingress-nginx --enable-helm > /tmp/ingress-nginx-198.yaml

echo "Ingress-nginx 배포 중..."
kubectl apply -f /tmp/ingress-nginx-198.yaml
...
# 3. Prometheus + Thanos Sidecar 배포
echo -e "${BLUE}════════════════════════════════════════${NC}"
echo -e "${BLUE}3/5 Prometheus + Thanos Sidecar 배포 중...${NC}"
echo -e "${BLUE}════════════════════════════════════════${NC}"

kubectl create namespace monitoring --dry-run=client -o yaml | kubectl apply -f -

echo "Prometheus + Thanos Sidecar 매니페스트 생성 중..."
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-198-edge/prometheus --enable-helm > /tmp/prometheus-198.yaml

echo "Prometheus + Thanos Sidecar 배포 중..."
kubectl apply -f /tmp/prometheus-198.yaml
...
echo -e "${GREEN}✓ Prometheus + Thanos Sidecar 배포 완료${NC}"
echo ""
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-198-edge/opensearch --enable-helm > /tmp/opensearch-198.yaml

echo "OpenSearch 배포 중..."
kubectl apply -f /tmp/opensearch-198.yaml
...
${KUSTOMIZE_CMD} ${PROJECT_ROOT}/deploy/overlays/cluster-198-edge/fluent-bit --enable-helm > /tmp/fluent-bit-198.yaml

echo "Fluent-bit 배포 중..."
kubectl apply -f /tmp/fluent-bit-198.yaml
...
echo ""
echo "Prometheus + Thanos Sidecar (monitoring):"
...
echo "  - Grafana: http://grafana.mkube-198.miribit.lab:30080"
echo "  - Alertmanager: http://alertmanager.mkube-198.miribit.lab:30080"
echo "  - OpenSearch: http://opensearch.mkube-198.miribit.lab:30080"
echo "  - Longhorn UI: http://longhorn.mkube-198.miribit.lab:30080"
echo ""
echo -e "${YELLOW}Thanos Sidecar NodePort:${NC}"
echo "  - gRPC: 192.168.101.198:30901"
echo ""
echo -e "${YELLOW}다음 단계:${NC}"
echo "  1. 다른 엣지 클러스터 배포: scripts/deploy-cluster-198-edge.sh"
echo "  2. 중앙 클러스터에서 멀티클러스터 쿼리 테스트"
echo ""
