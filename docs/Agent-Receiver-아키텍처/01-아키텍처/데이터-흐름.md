...
## 🔄 메트릭 데이터 흐름 (Write Path)

```mermaid
graph LR
    subgraph EdgeCluster["📡 Edge Cluster"]
        AGENT["Prometheus Agent<br/>Agent Mode<br/>Cluster-02/03/04"]
        WAL[("WAL<br/>Write-Ahead Log<br/>Local PVC")]
        QUEUE["Remote Write Queue<br/>In-Memory<br/>Capacity: 20000"]
    end

    subgraph CentralCluster["🎯 Central Cluster"]
        INGRESS["Nginx Ingress<br/>HTTP Router<br/>TLS Termination"]
        RECEIVER[("Thanos Receiver<br/>StatefulSet<br/>Hashring + TSDB")]
        REPLICATION["Replication<br/>gRPC<br/>RF=3"]
        TSDB[("TSDB<br/>Time Series DB<br/>2h blocks")]
    end

    S3[("💾 MinIO S3<br/>Object Storage<br/>Long-term Storage")]

    AGENT -->|"1. Scrape & Write<br/>15s interval"| WAL
    WAL -->|"2. Enqueue Samples<br/>Continuous"| QUEUE
    QUEUE -.->|"3. Remote Write<br/>HTTPS POST<br/>Batch: 5000 samples"| INGRESS
    INGRESS -->|"4. Route to Hashring<br/>HTTP:19291"| RECEIVER
    RECEIVER -->|"5. Replicate<br/>gRPC to 2 peers"| REPLICATION
    RECEIVER -->|"6. Write Local TSDB<br/>Samples insert"| TSDB
    TSDB -->|"7. Upload Block<br/>Every 2h<br/>S3 PUT"| S3

    style AGENT fill:#81c784,stroke:#388e3c,stroke-width:2px
    style WAL fill:#fff59d,stroke:#f57f17,stroke-width:2px
    style QUEUE fill:#fff59d,stroke:#f57f17,stroke-width:2px
    style RECEIVER fill:#4fc3f7,stroke:#0288d1,stroke-width:3px
    style TSDB fill:#4fc3f7,stroke:#0288d1,stroke-width:2px
...
```mermaid
graph TB
    GRAFANA["📊 Grafana<br/>Dashboard<br/>User Interface"]

    subgraph CentralCluster["🎯 Central Cluster"]
        QUERY["Thanos Query<br/>Query Engine<br/>PromQL + Dedup"]

        subgraph RecentData["최근 데이터 (<2h)"]
            RECEIVER[("Thanos Receiver<br/>StatefulSet<br/>Recent metrics")]
        end

        subgraph HistoricalData["과거 데이터 (>2h)"]
            STORE["Thanos Store<br/>Gateway<br/>Historical metrics"]
        end

        COMPACTOR["Thanos Compactor<br/>Downsampler<br/>5m, 1h resolution"]
    end

    S3[("💾 MinIO S3<br/>Object Storage<br/>TSDB Blocks")]

    GRAFANA -->|"1. PromQL Query<br/>HTTP:9090<br/>rate(cpu[5m])"| QUERY
    QUERY -->|"2a. Query Recent<br/>gRPC StoreAPI<br/>Last 2 hours"| RECEIVER
    QUERY -->|"2b. Query Historical<br/>gRPC StoreAPI<br/>Older than 2h"| STORE
    STORE <-->|"3. Fetch Blocks<br/>S3 GET<br/>Read TSDB blocks"| S3
    COMPACTOR -.->|"Background<br/>Downsample<br/>5m/1h aggregation"| S3
    RECEIVER -->|"5a. Return Recent<br/>Time series"| QUERY
    STORE -->|"5b. Return Historical<br/>Time series"| QUERY
    QUERY -->|"6. Deduplicate<br/>by replica label"| QUERY
    QUERY -->|"7. Result<br/>JSON response"| GRAFANA

    style GRAFANA fill:#66bb6a,stroke:#388e3c,stroke-width:2px
    style QUERY fill:#4fc3f7,stroke:#0288d1,stroke-width:3px
    style RECEIVER fill:#4fc3f7,stroke:#0288d1,stroke-width:2px
    style STORE fill:#4fc3f7,stroke:#0288d1,stroke-width:2px
    style COMPACTOR fill:#9575cd,stroke:#5e35b1,stroke-width:2px
    style S3 fill:#ff7043,stroke:#d84315,stroke-width:3px
```
